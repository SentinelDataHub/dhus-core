...
public class CacheNetworkServiceIT extends AbstractTestNGSpringContextTests
{
   @Autowired
   ...
   public void testNetworkServiceCache()
   {
      ...
      int a0 = cacheManager.getCache(count_cache).get(user.getUUID(), Integer.class);
      Assert.assertEquals(a0, r0);

      long r1 = networkService.getDownloadedSizeByUserSince(user, period);
      Assert.assertEquals(r1, 514);
      long a1 = cacheManager.getCache(size_cache).get(user.getUUID(), Long.class);
      Assert.assertEquals(a1, r1);

      // test cache eviction
      networkService.createDownloadUsage(64L, new Date(), user);
      Assert.assertNull(cacheManager.getCache(count_cache).get(user.getUUID(), Integer.class));
      Assert.assertNull(cacheManager.getCache(size_cache).get(user.getUUID(), Long.class));
   }

...
public class IncomingManager
{
   ...
   public HierarchicalDirectoryBuilder getIncomingBuilder ()
   {
      ...
      return new HierarchicalDirectoryBuilder(root.getAbsoluteFile(), incoming.getMaxFileNo());
   }
   ...
   {
      File root = new File(cfgManager.getArchiveConfiguration().getIncomingConfiguration().getPath())
            .getAbsoluteFile();
   ...
   class CheckIncomingWalker extends DirectoryWalker<File>
   {
      ...
      protected boolean handleDirectory (File directory, int depth, Collection<File> results)
      {
         ...
         try
         {
            found = productDao.find(query).iterator().hasNext();
         }
         catch (Exception e)
         {
            // Case of NullPointerException or SQLException: not found.
            // Found already set to false.
         }

...
public class CountryDao extends HibernateDao<Country, Long>
{
   ...
   public Country getCountryByName (final String name)
   {
      return getHibernateTemplate ().execute(session -> {
         Query query = session.createQuery("FROM Country WHERE name=?1");
   ...
   {
      return getHibernateTemplate ().execute(session -> {
         Query query = session.createQuery("FROM Country WHERE alpha2=?1");
         query.setParameter(1, alpha2, StandardBasicTypes.STRING);
         List list = query.list();
         return (Country) (list.isEmpty() ? null : list.get(0));
      });
   }
   ...
   {
      return getHibernateTemplate ().execute(session -> {
         Query query = session.createQuery("FROM Country WHERE alpha3=?1");
         query.setParameter(1, alpha3, StandardBasicTypes.STRING);
         List list = query.list();
         return (Country) (list.isEmpty() ? null : list.get(0));
      });
   }
   ...
   {
      return getHibernateTemplate ().execute(session -> {
         Query query = session.createQuery("FROM Country WHERE numeric=?1");
         query.setParameter(1, numeric, StandardBasicTypes.INTEGER);
         List list = query.list();
         return (Country) (list.isEmpty() ? null : list.get(0));
      });
   }

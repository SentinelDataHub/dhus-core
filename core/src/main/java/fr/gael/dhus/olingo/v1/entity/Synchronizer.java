...
public final class Synchronizer extends AbstractEntity
{
   ...
   public void updateFromEntry (ODataEntry odata_entry) throws ODataException
   {
      ...
      XMLGregorianCalendar last_ingestion_date =
            (XMLGregorianCalendar) props.remove(SynchronizerEntitySet.LAST_CREATION_DATE);
      ...
      Source source = syncConf.getSources().getSource().get(0);
      ProductSource productSource = PRODUCT_SOURCE_SERVICE.getProductSource(source);

         if (service_url != null && !service_url.isEmpty())
         {
            productSource.setUrl(service_url);
         }

         if (has_login)
         {
            productSource.setLogin(service_login);
         }

         if (has_password)
         {
            productSource.setPassword(service_password);
         }

         if (has_collec)
         {
            source.setSourceCollection(source_collection);
         }

         if (has_incoming)
         {
            productSource.setRemoteIncoming(remote_incoming);
         }
         ...
         if (has_last_date)
         {
            ...
            {
               source.setLastCreationDate(null);
            }
            else
            {
               source.setLastCreationDate(last_ingestion_date);
            }
         }
      try
      {
   ...
   public Map<String, Object> toEntityResponse (String root_url)
   {
      ...
      res.put(SynchronizerEntitySet.TIMEOUT_SKIPPED_PRODUCTS, this.syncConf.getTimeoutSkippedProducts());
         ...
         res.put(SynchronizerEntitySet.SERVICE_URL, productSource.getUrl());
         res.put(SynchronizerEntitySet.SERVICE_LOGIN, productSource.getLogin());
         res.put(SynchronizerEntitySet.SERVICE_PASSWORD,productSource.getPassword() != null ? "***" : null);
         res.put(SynchronizerEntitySet.SOURCE_COLLECTION,source.getSourceCollection());
         XMLGregorianCalendar last_created = source.getLastCreationDate();
   ...
   public Object getProperty (String prop_name) throws ODataException
   {
      ...
      {
         throw new ODataException("Cannot display OData Synchronizers");
      }
   ...
   {
      List<Source> sources = syncConf.getSources().getSource();
      if (sources.size() == 1)
      {
         return true;
      }

...
public class SynchronizerMap extends AbstractDelegatingMap<Long, Synchronizer>
{
   ...
   {
      Iterator<ProductSynchronizer> it = SYNC_SERVICE.getSynchronizerConfs(ProductSynchronizer.class);

      List<ProductSynchronizer> list = new ArrayList<>();
      it.forEachRemaining(sync -> {
         if (sync.getSources().getSource().size() == 1)
         {
            list.add(sync);
         }
      });

      final Iterator<ProductSynchronizer> itt = list.iterator();
            ...
            {  
               ...
               {
                  return new Synchronizer (itt.next ());
               }

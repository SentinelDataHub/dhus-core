...
public class SynchronizerService implements ISynchronizerService
{
   ...
   private Map<Long, ProductSynchronizerScheduler> mapSchedulers = new HashMap<Long, ProductSynchronizerScheduler>();;
   ...
   public void startSynchronization()
   {
      if (!executor.isRunning())
      {
         ...
         {
            Executors.newSingleThreadExecutor(new ThreadFactoryBuilder().setNameFormat("SynchronizerStart").build()).submit(() -> {
            LOGGER.info("Starting the Synchronizer {} ...",sc.getId());
            try
            {
   ...
   public void removeSynchronizer(long id)
   {
      ...
      if (sc != null)
      {
         ...
         finally
         {
            manager.delete(sc);
            removeScheduler(sc);
            LOGGER.info("Synchronizer#{} deleted by user {}", sc.getId(), secu.getCurrentUser().getUsername());
         }
      }
   }
   ...
   {
      Executors.newSingleThreadExecutor(new ThreadFactoryBuilder().setNameFormat("SynchronizerStart").build()).submit(() -> {
      LOGGER.info("Starting the Synchronizer {} ...",id);
      SynchronizerManager manager = cfgManager.getSynchronizerManager();
      SynchronizerConfiguration sc = manager.get(id);
      if (sc != null)
      {
         ...
         catch (InvokeSynchronizerException ex)
         {
            ...
            LOGGER.error("An error occured when traying to start the Synchronizer", ex);
         }
         if (!wasActive)
         {
            manager.update(sc);
            LOGGER.info("Synchronizer#{} started by user {}", sc.getId(), secu.getCurrentUser().getUsername());
         }
      }});
   }
   ...
   private void removeAllSynchronizerMetrics()
   {
      SynchronizerManager manager = cfgManager.getSynchronizerManager();
      List<SynchronizerConfiguration> listActiveSync = manager.getActiveSynchronizers();
      if (listActiveSync == null || listActiveSync.isEmpty())
      {
         metricRegistry.removeMatching((name, metric) -> name.startsWith("prod_sync"));
      }
   }
   ...
   private Synchronizer instanciate(SynchronizerConfiguration sc) throws InvokeSynchronizerException
   {
      try
      {
         ...
         {
            ODataProductSynchronizer odataProdSync = new ODataProductSynchronizer((ProductSynchronizer) sc, productSourceService);
            ProductSynchronizerScheduler scheduler = odataProdSync.getScheduler();
            if(scheduler != null)
            {
               mapSchedulers.put(odataProdSync.getId(), scheduler);
            }
            return odataProdSync;
         }
   ...
   private void removeScheduler(SynchronizerConfiguration sc)
   {
      if (sc instanceof ProductSynchronizer)
      {
         ProductSynchronizer prodSync = (ProductSynchronizer) sc;
         ProductSynchronizerScheduler scheduler = mapSchedulers.get(prodSync.getId());
         if (scheduler != null)
         {
            try
            {
               scheduler.unscheduleSource(prodSync);
               LOGGER.info("The source ranking for the Synchronizer {} is stopped", sc.getId());
            }
            catch (SchedulerException ex)
            {
               LOGGER.warn("Could not unschedule Synchronizer '{}'", prodSync.getId());
            }
         }
      }
   }

...
public class SynchronizerDataHandler extends AbstractSynchronizerDataHandler implements DataHandler
{
   ...
   protected Entity toOlingoEntity(SynchronizerConfiguration syncConf)
   {
      ...
      try
      {
         res.setId(new URI(SynchronizerModel.ABSTRACT_ENTITY_SET_NAME + "(" + syncConf.getId() + ")"));
      }
   ...
   protected void updateSpecificProperties(Entity updatedEntity, SynchronizerConfiguration sync)
   {
      if (sync instanceof ProductSynchronizer)
      {
         ...
         if (containsProperty(updatedEntity, ProductSynchronizerModel.PROPERTY_RANKING_SCHEDULE))
         {
            prodSync.setRankingSchedule((String) DataHandlerUtil.getPropertyValue(updatedEntity, 
         ...
         if (containsProperty(updatedEntity, ProductSynchronizerModel.PROPERTY_RETRYING_SOURCE_DELAY))
         {
            prodSync.setRetryingSourceDelay((Long)DataHandlerUtil.getPropertyValue(updatedEntity, 

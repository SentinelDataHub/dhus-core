...
import org.dhus.metrics.external.influx.InfluxMetricService;
...
{
   private static MetricsServiceInterface METRICS_SERVICE;
   ...
   static
   {
      ...
      catch (Throwable t) // bean not found => dhus-reporter not configured (not enabled)
      {
         try
         {
            METRICS_SERVICE = ApplicationContextProvider.getBean(InfluxMetricService.class);
         }
         catch (Throwable te)
         {
   ...
   {
      return METRICS_SERVICE.countMetrics(filterOption);
   }
         ...
         throws ODataApplicationException
   {
      ...
      MetricSQLVisitor sqlVisitor;
      if (METRICS_SERVICE instanceof MetricsService)
      {
         sqlVisitor = new MetricSQLVisitor(filterOption, orderByOption, topOption, skipOption);
      }
      else
      {
         sqlVisitor = new InfluxMetricsVisitor(filterOption, orderByOption, topOption, skipOption);
      }

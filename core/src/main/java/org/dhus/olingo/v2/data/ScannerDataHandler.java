...
public class ScannerDataHandler implements DataHandler
{
   ...
   public void updateEntityData(List<UriParameter> keyParameters, Entity updatedEntity, HttpMethod httpMethod) throws ODataApplicationException
   {
      ...
      updateCollection(updatedEntity, scannerInfo);
   ...
   private void updateCollection(Entity updatedEntity, ScannerInfo scannerInfo) throws ODataApplicationException
   {
      //Add collection
      Link collectionLink = updatedEntity.getNavigationLink(CollectionModel.ENTITY_SET_NAME);
      if (collectionLink != null)
      {
         EntityCollection collectionEntities = collectionLink.getInlineEntitySet();
         List<Entity> entities = collectionEntities.getEntities();
         for (Entity entity : entities)
         {
            ...
            List<String> collectionList = scannerInfo.getCollectionList();
            if (collectionList != null && !collectionList.contains(name))
            {
               if (COLLECTION_SVC.getCollectionByName(name) == null)
               {
                  throw new ODataApplicationException("Target collection does not exist: " + name,
                     HttpStatusCode.BAD_REQUEST.getStatusCode(), Locale.ENGLISH);
               }
               scannerInfo.addCollection(name);
            }
         }
      }
   }
   ...
   private FileScannerConf buildFileScannerConfig(Entity entity) throws ODataApplicationException
   {
      ...
      addCollection(fsi,entity);
      return fsi;
   }

   private void addCollection(ScannerInfo fsi, Entity entity) throws ODataApplicationException
   {
      updateCollection(entity, fsi);
   }

   private FtpScannerConf buildFtpScannerConfig(Entity entity) throws ODataApplicationException
   {
